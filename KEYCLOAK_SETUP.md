...
This document describes how to set up Keycloak for OIDC authentication and how to manage users in the Plain App Store.

## Keycloak Configuration

### 1. Create a Realm
Create a new realm (e.g., `plain-app-store`) or use an existing one.

### 2. Create a Client
1. Go to **Clients** -> **Create client**.
2. **Client ID**: `plain-app-store-client` (or as configured in `.env`).
...
BACKEND_OIDC_ISSUER=http://keycloak:8080/realms/plain-app-store
BACKEND_OIDC_CLIENT_ID=plain-app-store-client
...
KEYCLOAK_CLIENT_ID=plain-app-store-client
KEYCLOAK_CLIENT_SECRET=your-client-secret
KEYCLOAK_ISSUER=http://keycloak:8080/realms/plain-app-store
```
