...
export function AppCreationFlow({ existingApps, initialFormData = null, copySource = null }: AppCreationFlowProps) {
  ...
  const steps: Step[] = (() => {
    ...
    if (canManageGroups) next.push({ id: "groups", title: "Gruppen zuordnen", description: "Erstelle bei Bedarf eine Gruppe und ordne deine App zu.", icon: <Layers /> });
    if (isAdmin) next.push({ id: "admin", title: "Sichtbarkeit verwalten", description: "Hebe die App auf der Startseite hervor.", icon: <Sparkles /> });
  ...
  const reviewValue = (step: Step) => {
    ...
    switch (step.id) {
      ...
      case "groups": return groupIds.size ? `${groupIds.size} Gruppen` : "Nicht ergänzt";
      case "admin": return formData.isFeatured ? "Ausgezeichnet" : "Nicht ergänzt";
  ...
  const stage = () => {
    switch (current.id) {
      ...
      case "groups": { const createGroup = async () => { const name = newGroupName.trim(); if (!name || creatingGroup) return; setCreatingGroup(true); try { const response = await fetchApi("/app-groups", { method: "POST", body: JSON.stringify({ name }) }); if (!response.ok) { const error = await response.json().catch(() => ({})); throw new Error(error.message || "Gruppe konnte nicht erstellt werden."); } const group = await response.json() as { id: string; name: string; description?: string }; setGroups((previous) => [...previous, group]); setGroupIds((previous) => new Set([...previous, group.id])); setNewGroupName(""); toast.success("Gruppe erstellt."); } catch (error) { toast.danger(error instanceof Error ? error.message : "Gruppe konnte nicht erstellt werden."); } finally { setCreatingGroup(false); } }; return <div className="space-y-5"><TextField onChange={setNewGroupName}><Label>Neue Gruppe</Label><Input value={newGroupName} placeholder="z. B. Digitale Verwaltung" onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); void createGroup(); } }} /></TextField><Button variant="secondary" isDisabled={!newGroupName.trim()} isPending={creatingGroup} onPress={() => void createGroup()}>Gruppe erstellen</Button><div className="space-y-2">{groups.length ? groups.map((group) => <div key={group.id} className="flex items-center justify-between rounded-2xl border border-border p-4"><div><p className="font-semibold">{group.name}</p><p className="text-xs text-muted">{group.description}</p></div><Switch isSelected={groupIds.has(group.id)} onChange={(value) => setGroupIds((previous) => { const next = new Set(previous); if (value) next.add(group.id); else next.delete(group.id); return next; })}><Switch.Content><Switch.Control><Switch.Thumb /></Switch.Control></Switch.Content></Switch></div>) : <p className="text-sm text-muted">Noch keine Gruppen vorhanden.</p>}</div></div>; }
      case "admin": return <div className="flex items-center justify-between rounded-2xl border border-border p-4"><div><p className="font-semibold">Ausgezeichnet</p><p className="text-sm text-muted">Auf der Startseite hervorheben.</p></div><Switch isSelected={!!formData.isFeatured} onChange={(isFeatured) => setFormData((previous) => ({ ...previous, isFeatured }))}><Switch.Content><Switch.Control><Switch.Thumb /></Switch.Control></Switch.Content></Switch></div>;
