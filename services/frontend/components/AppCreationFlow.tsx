...
import {
  Alert, Button, Chip, FieldError, Input, Label, ProgressBar, Radio, RadioGroup,
  Surface, Switch, TextArea, TextField, toast,
} from "@heroui/react";
import {
...
export function AppCreationFlow({ existingApps, initialFormData = null, copySource = null }: AppCreationFlowProps) {
  ...
  const [branches, setBranches] = useState<Branches>(() => ({
    repository: !!initialFormData?.repositories?.length, deployment: initialFormData?.hasDeploymentAssistant !== false,
  ...
  const steps: Step[] = (() => {
    const next: Step[] = [
      ...
      { id: "logo", title: "Gib ihr ein Gesicht", description: "Lade ein Logo hoch oder wähle ein passendes Symbol.", icon: <ImagePlus /> },
  ...
  const validCurrent = () => {
    ...
    if (current.id === "categories") return !!formData.categories?.length;
    if (current.id === "description") return !nonDraft || !!formData.description?.trim();
    return true;
  };
  const move = async (direction: 1 | -1) => {
    if (direction === 1 && !validCurrent()) { setSaveError(current.id === "categories" ? "Wähle mindestens eine Kategorie." : "Bitte fülle die erforderlichen Angaben aus."); return; }
    if (direction === 1 && current.id === "name") { try { await save(); } catch { return; } }
    const target = steps[currentIndex + direction];
    if (target) { setCurrentId(target.id); if (direction === 1 && returnToReview) { setReturnToReview(false); } }
  };
  ...
  const stage = () => {
    switch (current.id) {
      ...
      case "categories": return <div className="space-y-4"><div className="flex flex-wrap gap-2">{CATEGORIES.map((category) => { const selected = formData.categories?.includes(category); return <Button key={category} variant={selected ? "primary" : "secondary"} size="sm" onPress={() => setFormData((previous) => ({ ...previous, categories: selected ? previous.categories?.filter((item) => item !== category) : [...(previous.categories || []), category] }))}>{category}</Button>; })}</div><div className="flex gap-2"><Input value={categoryInput} onChange={(event) => setCategoryInput(event.target.value)} placeholder="Eigene Kategorie" onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); setFormData((previous) => ({ ...previous, categories: Array.from(new Set([...(previous.categories || []), categoryInput.trim()])).filter(Boolean) })); setCategoryInput(""); } }} /><Button variant="secondary" onPress={() => { setFormData((previous) => ({ ...previous, categories: Array.from(new Set([...(previous.categories || []), categoryInput.trim()])).filter(Boolean) })); setCategoryInput(""); }}>Hinzufügen</Button></div></div>;
      case "status": return <EditorStatusPicker value={formData.status} onChange={(status) => setFormData((previous) => ({ ...previous, status }))} />;
      case "description": return <TextField isRequired={nonDraft} onChange={(description) => setFormData((previous) => ({ ...previous, description }))}><Label>Kurzbeschreibung</Label><TextArea autoFocus value={formData.description || ""} placeholder="Wofür steht die App? Welche Wirkung hat sie im Alltag?" className="min-h-52" /></TextField>;
      ...
      case "repo-project": return <div className="grid gap-5 sm:grid-cols-2"><TextField onChange={(providerKey) => setRepo((previous) => ({ ...previous, providerKey }))}><Label>Provider</Label><Input list="creation-providers" value={repo.providerKey} placeholder="Provider auswählen" /><datalist id="creation-providers">{providers.map((provider) => <option key={provider.key} value={provider.key}>{provider.label}</option>)}</datalist></TextField><TextField onChange={(projectPath) => setRepo((previous) => ({ ...previous, projectPath }))}><Label>Projektpfad</Label><Input value={repo.projectPath} placeholder="gruppe/projekt" /></TextField></div>;
      ...
      case "reuse": return <div className="space-y-5"><SectionGate title="Nachnutzung" description="Andere Stellen können die App als bestehende Lösung verwenden." enabled={!!formData.isReuse} onChange={(value) => setFormData((previous) => ({ ...previous, isReuse: value }))} icon={<Layers className="h-5 w-5" />} />{formData.isReuse && <TextField onChange={(reuseRequirements) => setFormData((previous) => ({ ...previous, reuseRequirements }))}><Label>Voraussetzungen und Grenzen</Label><TextArea value={formData.reuseRequirements || ""} placeholder="Welche Stellen können die App nachnutzen?" /></TextField>}</div>;
      ...
      case "deployment-methods": return <div className="grid gap-3 sm:grid-cols-3">{(["Docker", "Docker Compose", "Helm"] as const).map((label, index) => { const key = index === 0 ? "showDocker" : index === 1 ? "showCompose" : "showHelm"; const selected = formData[key] !== false; return <Button key={key} variant={selected ? "primary" : "secondary"} className="h-24" onPress={() => setFormData((previous) => ({ ...previous, [key]: !selected }))}>{label}</Button>; })}</div>;
      case "deployment-docker": return <div className="space-y-5"><TextField onChange={(dockerRepo) => setFormData((previous) => ({ ...previous, dockerRepo }))}><Label>Docker Image</Label><Input value={formData.dockerRepo || ""} placeholder="image:latest" /></TextField><TextField onChange={(customDockerCommand) => setFormData((previous) => ({ ...previous, customDockerCommand }))}><Label>Docker-Kommando</Label><TextArea value={formData.customDockerCommand || ""} placeholder="docker run …" /></TextField><TextField onChange={(customDockerNote) => setFormData((previous) => ({ ...previous, customDockerNote }))}><Label>Hinweis</Label><Input value={formData.customDockerNote || ""} placeholder="Ports, Secrets oder Startparameter" /></TextField></div>;
      case "deployment-compose": return <div className="space-y-5"><TextField onChange={(customComposeCommand) => setFormData((previous) => ({ ...previous, customComposeCommand }))}><Label>Compose-Setup</Label><TextArea value={formData.customComposeCommand || ""} placeholder="services:\n  app: …" /></TextField><TextField onChange={(customComposeNote) => setFormData((previous) => ({ ...previous, customComposeNote }))}><Label>Hinweis</Label><Input value={formData.customComposeNote || ""} /></TextField></div>;
      case "deployment-helm": return <div className="space-y-5"><TextField onChange={(helmRepo) => setFormData((previous) => ({ ...previous, helmRepo }))}><Label>Helm Repository</Label><Input value={formData.helmRepo || ""} placeholder="oci://…" /></TextField><TextField onChange={(customHelmCommand) => setFormData((previous) => ({ ...previous, customHelmCommand }))}><Label>Helm-Kommando</Label><TextArea value={formData.customHelmCommand || ""} placeholder="helm install …" /></TextField><TextField onChange={(customHelmValues) => setFormData((previous) => ({ ...previous, customHelmValues }))}><Label>Values.yaml</Label><TextArea value={formData.customHelmValues || ""} /></TextField><TextField onChange={(customHelmNote) => setFormData((previous) => ({ ...previous, customHelmNote }))}><Label>Hinweis</Label><Input value={formData.customHelmNote || ""} placeholder="Zusatz für die Einführung" /></TextField></div>;
      ...
      case "banner-content": return <div className="space-y-5"><RadioGroup value={formData.bannerType || "info"} onChange={(bannerType) => setFormData((previous) => ({ ...previous, bannerType: bannerType as AppConfig["bannerType"] }))} orientation="horizontal">{["info", "warning", "danger"].map((type) => <Radio key={type} value={type}>{type === "info" ? "Info" : type === "warning" ? "Warnung" : "Kritisch"}</Radio>)}</RadioGroup><TextField onChange={(bannerTitle) => setFormData((previous) => ({ ...previous, bannerTitle }))}><Label>Überschrift</Label><Input value={formData.bannerTitle || ""} placeholder="Optional" /></TextField><TextField onChange={(bannerText) => setFormData((previous) => ({ ...previous, bannerText }))}><Label>Hinweistext</Label><TextArea value={formData.bannerText || ""} /></TextField></div>;
      ...
      case "related-apps": { const available = existingApps.filter((app) => !relatedApps.some((related) => related.id === app.id) && app.id !== draftId && (!relatedSearch || app.name.toLowerCase().includes(relatedSearch.toLowerCase()) || app.id.includes(relatedSearch))); return <div className="space-y-4"><Input value={relatedSearch} onChange={(event) => setRelatedSearch(event.target.value)} placeholder="App suchen" />{relatedApps.length > 0 && <div className="flex flex-wrap gap-2">{relatedApps.map((app) => <Chip key={app.id} variant="soft">{app.name}</Chip>)}</div>}<div className="space-y-2">{available.slice(0, 8).map((app) => <Button key={app.id} variant="secondary" fullWidth className="justify-between" onPress={() => addRelated(app)}>{app.name}<ArrowRight className="h-4 w-4" /></Button>)}</div></div>; }
      ...
      case "markdown": return <TextField onChange={(markdownContent) => setFormData((previous) => ({ ...previous, markdownContent }))}><Label>Dokumentation</Label><TextArea value={formData.markdownContent || ""} placeholder={`# ${formData.name || "App"}\n\nBeschreibe die App …`} className="min-h-[28rem] font-mono" /></TextField>;
      case "review": return <div className="space-y-4"><Alert color="accent"><Alert.Content><Alert.Title>Bereit zum Erstellen</Alert.Title><Alert.Description>Prüfe die wichtigsten Angaben. Du kannst jeden Abschnitt direkt wieder öffnen.</Alert.Description></Alert.Content></Alert><div className="grid gap-3 sm:grid-cols-2">{[["name", "Name", formData.name], ["logo", "Logo", formData.icon ? "Gewählt" : "Nicht gewählt"], ["categories", "Kategorien", formData.categories?.join(", ")], ["status", "Status", formData.status], ["description", "Kurzbeschreibung", formData.description], ["metadata", "Metadaten", formData.license]].map(([id, label, value]) => <button key={String(id)} type="button" onClick={() => edit(String(id))} className="rounded-2xl border border-border bg-surface p-4 text-left hover:border-accent/40"><p className="text-xs font-bold uppercase tracking-wider text-muted">{label}</p><p className="mt-1 truncate font-semibold text-foreground">{value || "Noch nicht ergänzt"}</p></button>)}</div></div>;
  ...
  const handleStageKeyDown = (event: React.KeyboardEvent<HTMLElement>) => {
    ...
    else if (returnToReview) setCurrentId("review");
    else void move(1);
  };
