...
function normalizeGitLabFormState(
  ...
  const selectedProvider = integration?.availableProviders?.find(
  ...
  return {
    ...
    readmePath:
      integration?.readmePath || selectedProvider?.defaultReadmePath || "",
    helmValuesPath:
      integration?.helmValuesPath ||
      selectedProvider?.defaultHelmValuesPath ||
      "",
    composeFilePath:
      integration?.composeFilePath ||
      selectedProvider?.defaultComposeFilePath ||
      "",
  };
}
...
export function AppEditorForm({
  ...
  const [iconUrlInput, setIconUrlInput] = useState(
    isImageAssetSource(initialEditorValues?.icon)
      ? initialEditorValues?.icon || ""
      : "",
  );
  ...
  const [gitLabIntegration, setGitLabIntegration] =
    useState<GitLabIntegrationState | null>(null);
  const [gitLabForm, setGitLabForm] = useState<GitLabFormState>(
    defaultGitLabFormState,
  );
  const [loadingGitLab, setLoadingGitLab] = useState(
    !isNew && !!initialApp && canManageAppStructure,
  );
  const [savingGitLab, setSavingGitLab] = useState(false);
  const [syncingGitLab, setSyncingGitLab] = useState(false);
  const [gitLabError, setGitLabError] = useState<string | null>(null);
  const [creationProviders, setCreationProviders] = useState<
    GitLabProviderSummary[]
  >([]);
    ...
    const interval = setInterval(async () => {
      ...
      try {
        if (!autoSaveAppIdRef.current) {
          ...
          const res = await fetchApi(`/apps/${autoSaveAppIdRef.current}`, {
            method: "PUT",
            body: JSON.stringify(body),
          });
  ...
  const isIdTaken =
    isNew &&
    !!formData.id?.trim() &&
    existingApps.some((a) => a.id === formData.id?.trim());
        ...
        if (!response.ok) {
          ...
          throw new Error(
            (error as { message?: string }).message ||
              "Repository-Daten konnten nicht geladen werden.",
          );
        }
  ...
  const handleCreationGitLabSync = async () => {
    if (!gitLabForm.projectPath?.trim() || !gitLabForm.providerKey?.trim())
      return;
    ...
    try {
      ...
      // Sync
      const syncRes = await fetchApi(`/apps/${appId}/repository/sync`, {
        method: "POST",
      });
      if (!syncRes.ok) {
        ...
        throw new Error(
          (err as { message?: string }).message ||
            "Repository-Synchronisation fehlgeschlagen.",
        );
      }
      ...
      if (snapshot) {
        setFormData((prev) => {
          ...
          if (snapshot.description?.trim())
            next.description = snapshot.description.trim();
          ...
          if (
            snapshot.projectWebUrl &&
            !repos.some((r) => r.url === snapshot.projectWebUrl)
          ) {
  ...
  const getEffectiveAppId = () =>
    initialApp?.id || preCreatedAppId || autoSaveAppIdRef.current || null;
  const applyGitLabProviderSelection = (providerKey: string) => {
    ...
    setGitLabForm((previous) => ({
      ...
      providerKey,
      readmePath:
        previous.readmePath || selectedProvider?.defaultReadmePath || "",
      helmValuesPath:
        previous.helmValuesPath ||
        selectedProvider?.defaultHelmValuesPath ||
        "",
      composeFilePath:
  ...
  const ensureEditableAppId = async () => {
    ...
    if (!formData.name?.trim() || !formData.id?.trim()) {
      throw new Error(
        "Bitte zuerst Name und ID ausfüllen, damit ein Entwurf für Verknüpfungen angelegt werden kann.",
      );
    }
    ...
    const body: Partial<AppConfig> = {
      ...
      techStack: Array.isArray(formData.techStack) ? formData.techStack : [],
      repositories: sanitizeLinks(
        formData.repositories,
      ) as AppConfig["repositories"],
      customLinks: sanitizeLinks(
        formData.customLinks,
      ) as AppConfig["customLinks"],
    ...
    if (!response.ok) {
      ...
      throw new Error(
        (error as { message?: string }).message ||
          "Entwurf konnte nicht automatisch angelegt werden.",
      );
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSaveError(
          (err as { message?: string }).message || "Speichern fehlgeschlagen",
        );
        toast.danger(
          (err as { message?: string }).message || "Speichern fehlgeschlagen",
        );
  ...
  const handleAddRelated = async (app: AppConfig) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          (error as { message?: string }).message ||
            "Verwandte App konnte nicht verknüpft werden.",
        );
      }
  ...
  const handleRemoveRelated = async (relatedId: string) => {
    ...
    const response = await fetchApi(`/apps/${appId}/related/${relatedId}`, {
      method: "DELETE",
    });
    if (response.ok) {
  ...
  const handleSyncGitLab = async () => {
    ...
    try {
      ...
      if (integration.approvalRequired) {
        ...
        toast.success(
          integration.lastSyncStatus === "warning"
            ? "Repository synchronisiert, aber mit Hinweisen."
  ...
  const handleApproveGitLab = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          (data as { message?: string }).message ||
            "Repository-Änderung konnte nicht freigegeben werden.",
        );
      }
  ...
  const handleApplyGitLabMetadata = () => {
    ...
    setFormData((prev) => {
      ...
      if (
        snapshot.projectWebUrl &&
        !nextRepositories.some((link) => link.url === snapshot.projectWebUrl)
      ) {
        nextRepositories.push({
          label: "Repository",
          url: snapshot.projectWebUrl,
        });
      }
  ...
  const handleApplyGitLabDeployment = () => {
    ...
    setFormData((prev) => ({
      ...
      showCompose: snapshot.composeFileContent?.trim()
        ? true
        : prev.showCompose,
      customHelmValues:
        snapshot.helmValuesContent?.trim() || prev.customHelmValues,
      customComposeCommand:
        snapshot.composeFileContent?.trim() || prev.customComposeCommand,
    }));
  ...
  const isDraft = isDraftStatus(
    formData.status || (isNew ? DRAFT_STATUS : undefined),
  );
  ...
  const canSave =
    draftRequiredItems.every((item) => item.done) &&
    (!requiresExpandedDetails || rolloutItems.every((item) => item.done));
  const requiredDoneCount = requiredItems.filter((item) => item.done).length;

  /* Build metadata for Fachliche Details tab */
  const fieldValueMap = new Map(
    (formData.customFields ?? []).map((f) => [f.key, f.value]),
  );
  const metaFields = (settings.detailFields ?? []).map((def) => ({
    key: def.key,
    label: def.label,
    value: fieldValueMap.get(def.key) || "",
  ...
  const hasGitLabProviders =
    (gitLabIntegration?.availableProviders?.length || 0) > 0;
  ...
  const isCreateStepCompleted = (stepIndex: number) => {
    switch (stepIndex) {
      ...
      case 3:
        return (
          !!formData.reuseRequirements?.trim() ||
          !!formData.isReuse ||
      ...
      case 4:
        return (
          (formData.liveDemos?.some((demo) => demo.url?.trim()) ?? false) ||
          (formData.repositories?.some((repository) =>
            repository.url?.trim(),
          ) ??
            false) ||
  ...
  const completedCreateStepCount = createSteps.filter((_, index) =>
    isCreateStepCompleted(index),
  ).length;
    ...
    switch (currentCreateStep) {
      case 0:
        return [
          ...
          {
            label: "Mindestens eine Kategorie",
            done: (formData.categories?.length ?? 0) > 0,
          },
        ];
      case 1:
        return [
      ...
      case 2:
        return [
          {
            label: "Provider gewählt (optional)",
            done: !!gitLabForm.providerKey?.trim(),
          },
          {
            label: "Projektpfad eingetragen (optional)",
            done: !!gitLabForm.projectPath?.trim(),
          },
          {
            label: "Wird nach Speichern verknüpft",
            done: !!gitLabForm.projectPath?.trim(),
          },
        ];
      case 3:
        return [
          {
            label: "Nachnutzung eingeordnet (optional)",
            done: formData.isReuse !== undefined,
          },
          {
            label: "Deployment Assistant konfiguriert",
            done: formData.hasDeploymentAssistant !== undefined,
          },
          {
            label: "Installationswege gewählt",
            done:
              formData.showDocker !== false ||
              formData.showCompose !== false ||
              formData.showHelm !== false,
      ...
      case 4:
        return [
          {
            label: "Live-Zugang (optional)",
            done: !!(formData.liveDemos || []).some((demo) => demo.url?.trim()),
          },
          {
            label: "Repository oder Ressource (optional)",
            done:
              (formData.repositories?.length ?? 0) > 0 ||
              (formData.customLinks?.length ?? 0) > 0 ||
              !!formData.docsUrl?.trim(),
          },
          {
            label: "Dokumentationslink (optional)",
            done: !!formData.docsUrl?.trim(),
          },
        ];
      case 5:
        return [
          {
            label: "Herausgeber (optional)",
            done: !!formData.authority?.trim(),
          },
          {
            label: "Fachliche Details gepflegt",
            done: (formData.customFields?.length ?? 0) > 0,
          },
          {
            label: "Banner bei Bedarf",
            done: !formData.bannerText || !!formData.bannerText.trim(),
          },
        ];
      case 6:
        return [
          {
            ...
            done: !!formData.markdownContent?.trim(),
          },
          { label: "Entwurf speicherbar", done: canSave },
          {
            label: "Bereit für ersten Review",
            done: requiredDoneCount >= draftRequiredItems.length,
          },
        ];
      default:
        return [];
  ...
  const buildSnapshot = (nextFormData: Partial<AppConfig>) =>
    JSON.stringify({
      ...
      <div className="max-w-6xl mx-auto pb-28">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        <span
                          className={`inline-flex h-8 w-8 items-center justify-center rounded-xl border text-sm font-semibold transition-colors ${
                            isActive
                        ...
                        <span
                          className={`text-[11px] font-bold uppercase tracking-[0.2em] ${isActive ? "text-white/75" : "text-muted/70"}`}
                        >
                          {index + 1}
                        </span>
                      </div>
                      <p
                        className={`mt-3 text-sm font-semibold ${isActive ? "text-white" : "text-foreground"}`}
                      >
                        {step.label}
                      </p>
                      <p
                        className={`mt-1 text-[11px] leading-relaxed ${isActive ? "text-white/75" : "text-muted"} ${isActive ? "block" : "hidden md:block"}`}
                      >
                        {step.hint}
                      </p>
                    </button>
        ...
        {isCopyFlow && (
          <section className="mt-4 rounded-[1.5rem] border border-accent/20 bg-accent/5 px-5 py-4 shadow-sm md:px-6">
            <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-accent/80">
              Kopie vorbereitet
            </p>
            <p className="mt-2 text-sm text-foreground">
              Die Vorlage{" "}
              <span className="font-semibold">{copySource.name}</span> wurde als
              neuer Entwurf übernommen. Beziehungen, Repository-Verknüpfungen
              und Systemmetadaten werden nicht mitkopiert.
            </p>
          </section>
        )}
          ...
          >
            <div className="mb-6 flex flex-col gap-3 border-b border-border pb-4 md:flex-row md:items-center md:justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[11px] font-bold uppercase tracking-[0.24em] text-muted/70">
                    {currentCreatePhase}
                  </p>
                  <h2 className="mt-1 text-xl font-bold text-foreground">
                    {currentCreateStepDef.label}
                  </h2>
                  <p className="mt-0.5 text-sm text-muted">
                    {currentCreateStepDef.hint}
                  </p>
                </div>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                <span className="rounded-full border border-border bg-surface-secondary px-3 py-1 text-xs font-semibold text-foreground shadow-sm">
                  {completedCreateStepCount} von {createSteps.length} Schritten
                  vorbereitet
                </span>
              ...
              <div className="space-y-6">
                <div className="grid gap-6 lg:grid-cols-[19rem_minmax(0,1fr)]">
                  <div className="space-y-4 rounded-3xl border border-accent/15 bg-accent/5 p-5">
                    <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-accent/80">
                      App-Icon
                    </p>
                    <button
                      type="button"
                          ...
                          {ICON_OPTIONS.map(({ emoji, label }) => (
                            <button
                              ...
                              className={`flex h-10 w-10 items-center justify-center rounded-xl border text-xl transition-all ${
                                formData.icon === emoji
                                  ? "border-accent bg-accent/10 shadow-sm scale-105"
                                  : "border-border bg-surface-secondary hover:border-accent/30"
                              }`}
                            >
                              {emoji}
                            </button>
                      ...
                      <div>
                        ...
                        >
                          ...
                          <Input
                            value={formData.id || ""}
                            placeholder="z.B. digi-sign-pro"
                            className="bg-field-background font-mono"
                          />
                        </TextField>
                        <p
                          className={`mt-2 text-xs ${isIdTaken ? "text-danger" : "text-muted"}`}
                        >
                          {isIdTaken
                            ? "Diese ID ist bereits vergeben."
                            : "Die ID wird automatisch aus dem Namen vorgeschlagen und bleibt editierbar."}
                        </p>
                      </div>
                    </div>
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-1 text-xs text-muted">
                            Mindestens eine Kategorie ist für die Einordnung
                            erforderlich.
                          </p>
                        </div>
                        <button
                      ...
                      <div className="flex flex-wrap gap-2">
                        {(formData.tags || []).map((tag) => (
                          <Chip
                            ...
                            size="sm"
                            variant="soft"
                            className="text-xs font-medium bg-surface-secondary border border-border"
                          >
                            {tag}
                            <button
                              type="button"
            ...
            {currentCreateStep === 2 && (
              <div className="space-y-6">
                <div className="rounded-3xl border border-border bg-surface-secondary/45 p-5">
                  ...
                  <p className="mt-1 text-xs text-muted">
                    Optional: Verknüpfen Sie die App mit einem Repository.
                    README, Metadaten und Deployment-Dateien können direkt hier
                    importiert werden, bevor Sie die nächsten Schritte
                    ausfüllen.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                  <div className="rounded-2xl border border-border bg-surface p-4">
                    ...
                    {creationProviders.length > 0 ? (
                      <select
                        ...
                        onChange={(e) =>
                          applyGitLabProviderSelection(e.target.value)
                        }
                  ...
                  <div className="rounded-2xl border border-border bg-surface p-4">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setGitLabForm((p) => ({ ...p, branch: e.target.value }))
                      }
                  ...
                  <div className="rounded-2xl border border-border bg-surface p-4">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setGitLabForm((p) => ({
                          ...p,
                          composeFilePath: e.target.value,
                        }))
                      }
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  {!gitLabForm.projectPath?.trim() && (
                    <p className="text-xs text-muted">
                      Kein Repository angegeben – dieser Schritt wird
                      übersprungen.
                    </p>
                  )}
                </div>
                  ...
                  <div className="rounded-3xl border border-success/20 bg-success/5 p-5">
                    <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-sm font-semibold text-foreground">
                          Synchronisation abgeschlossen – Daten wurden
                          übernommen
                        </p>
                      </div>
                      {gitLabSnapshot.syncedAt && (
                        <span className="text-xs text-muted">
                          {new Date(gitLabSnapshot.syncedAt).toLocaleString(
                            "de-DE",
                          )}
                        </span>
                      )}
                    ...
                    <ul className="mt-3 space-y-1 pl-6 text-xs text-muted">
                      {gitLabSnapshot.readmeContent?.trim() && (
                        <li>README → Dokumentation</li>
                      )}
                      {(gitLabSnapshot.description?.trim() ||
                        gitLabSnapshot.topics?.length ||
                        gitLabSnapshot.license?.trim()) && (
                        <li>Beschreibung, Lizenz, Topics → Kurzprofil</li>
                      )}
                      {gitLabSnapshot.projectWebUrl && (
                        <li>Repository-Link → Ressourcen</li>
                      )}
                      {gitLabSnapshot.helmValuesContent?.trim() && (
                        <li>Helm Values → Bereitstellung</li>
                      )}
                      {gitLabSnapshot.composeFileContent?.trim() && (
                        <li>Compose-Datei → Bereitstellung</li>
                      )}
                    </ul>
                  </div>
                )}
                ...
                <div className="rounded-3xl border border-border bg-surface p-5">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-muted">
                        Aktivieren Sie diesen Modus, wenn die App als bestehende
                        Lösung mitgenutzt werden kann.
                      </p>
                    </div>
                    <Switch
                  ...
                  <textarea
                    ...
                    onChange={(event) =>
                      setFormData((previous) => ({
                        ...previous,
                        reuseRequirements: event.target.value,
                      }))
                    }
                  />
                </div>
                  ...
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      ...
                      <p className="mt-1 max-w-2xl text-sm leading-relaxed text-muted">
                        Wählen Sie, welche Installationswege im Store gezeigt
                        werden sollen. Sie können die technischen Angaben auch
                        später noch pro Weg ergänzen.
                      </p>
                    </div>
                    <Switch
                      ...
                      {[
                        {
                          ...
                          label: "Helm",
                          description:
                            "Chart und Values für Kubernetes-basierte Installationen.",
                          icon: <Server className="w-4 h-4" />,
                        },
                        {
                          key: "showCompose",
                          label: "Compose",
                          description:
                            "Mehrere Container mit gemeinsamem Setup.",
                        ...
                        },
                        {
                          ...
                          label: "Docker",
                          description:
                            "Direkter Container-Start für einfache Deployments.",
                          icon: <Terminal className="w-4 h-4" />,
                        },
                      ].map(({ key, label, description, icon }) => {
                        const active =
                          formData[key as keyof AppConfig] !== false;
                        return (
                          <button
                            key={key}
                            type="button"
                            onClick={() =>
                              setFormData((previous) => {
                                const currentlyActive =
                                  previous[key as keyof AppConfig] !== false;
                            ...
                            className={`flex min-h-36 flex-col rounded-[1.75rem] border-2 p-5 text-left transition-all ${
                              active
                                ? "border-accent bg-accent/6 text-accent shadow-sm shadow-accent/10"
                                : "border-border bg-surface-secondary/55 text-muted hover:border-border/80"
                            }`}
                          >
                            <div className="flex items-start justify-between gap-3">
                              <span
                                className={`inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border ${active ? "border-accent/20 bg-accent/12 text-accent" : "border-border bg-surface text-muted"}`}
                              >
                                {icon}
                              </span>
                              <span
                                className={`rounded-full border px-2.5 py-1 text-[11px] font-semibold ${active ? "border-accent/20 bg-accent/12 text-accent" : "border-border bg-surface text-muted"}`}
                              >
                            ...
                            <div className="mt-5 space-y-2">
                              <p
                                className={`text-xl font-semibold leading-tight ${active ? "text-accent" : "text-foreground"}`}
                              >
                                {label}
                              </p>
                              <p
                                className={`text-sm leading-relaxed ${active ? "text-accent/75" : "text-muted"}`}
                              >
                                {description}
                              </p>
                            </div>
                ...
                {formData.hasDeploymentAssistant !== false && (
                  <div className="space-y-4">
                      ...
                      <div className="rounded-3xl border border-border bg-surface-secondary/40 p-5">
                        ...
                        <div className="grid gap-4 lg:grid-cols-2">
                          ...
                          >
                            ...
                            <Input
                              ...
                              placeholder="Zusatz für die Einführung"
                        ...
                        <div className="mt-4 grid gap-4 lg:grid-cols-2">
                          ...
                          >
                            ...
                            <TextArea
                              value={formData.customHelmCommand || ""}
                              className="bg-field-background font-mono text-sm"
                              placeholder={`helm install ${formData.id || "app"} repo/${formData.id || "app"}`}
                            />
                          </TextField>
                          <TextField
                            onChange={(value) =>
                          ...
                          >
                            ...
                            <TextArea
                              value={formData.customHelmValues || ""}
                              className="bg-field-background font-mono text-sm"
                              placeholder="image:\n  tag: latest"
                            />
                          </TextField>
                    ...
                    {formData.showCompose !== false && (
                      <div className="rounded-3xl border border-border bg-surface-secondary/40 p-5">
                        <div className="mb-4 flex items-center gap-2 text-sm font-semibold text-foreground">
                          <Terminal className="w-4 h-4 text-accent" /> Docker
                          Compose
                          {gitLabSnapshot?.composeFileContent?.trim() && (
                            <span className="ml-1 inline-flex items-center gap-1 rounded-full border border-accent/20 bg-accent/10 px-2 py-0.5 text-[10px] font-semibold text-accent">
                              <GitBranch className="h-3 w-3" /> via Repository
                        ...
                        <div className="grid gap-4 lg:grid-cols-2">
                          ...
                          >
                            ...
                            <TextArea
                              value={formData.customComposeCommand || ""}
                              className="bg-field-background font-mono text-sm"
                          ...
                          >
                            ...
                            <Input
                              value={formData.customComposeNote || ""}
                              className="bg-field-background text-sm"
                              placeholder="Netzwerk, Volumes oder Voraussetzungen"
                            />
                          </TextField>
                        </div>
                      </div>
                      ...
                      <div className="rounded-3xl border border-border bg-surface-secondary/40 p-5">
                        ...
                        <div className="grid gap-4 lg:grid-cols-2">
                          ...
                          >
                            ...
                            <Input
                              value={formData.dockerRepo || ""}
                              placeholder="image:latest"
                              className="bg-field-background font-mono text-sm"
                            />
                          </TextField>
                          <TextField
                          ...
                          >
                            ...
                            <Input
                              value={formData.customDockerNote || ""}
                              className="bg-field-background text-sm"
                              placeholder="Ports, Secrets oder Startparameter"
                            />
                          </TextField>
                        </div>
                        <TextField
                          onChange={(value) =>
                            setFormData((previous) => ({
                              ...previous,
                              customDockerCommand: value,
                            }))
                          }
                        >
                          <Label className="mb-1 mt-4 text-[10px] font-bold uppercase tracking-wider text-muted">
                            Docker-Kommando
                          </Label>
                          <TextArea
                            value={formData.customDockerCommand || ""}
                            className="bg-field-background font-mono text-sm"
                            placeholder={`docker run -d --name ${formData.id || "app"} ...`}
                          />
                        </TextField>
                      </div>
                    )}
            ...
            {currentCreateStep === 4 && (
              <div className="space-y-6">
                <div className="rounded-3xl border border-border bg-surface-secondary/45 p-5">
                  ...
                  <p className="mt-1 text-xs text-muted">
                    Sammeln Sie die wichtigsten Einstiege für Live-Zugänge, Code
                ...
                <div className="space-y-6">
                  <LinkListEditor
                    ...
                    onChange={(liveDemosValue) =>
                      setFormData((previous) => ({
                        ...previous,
                        liveDemos: liveDemosValue,
                      }))
                    }
                    addLabel="Hinzufügen"
                  ...
                  <div className="flex items-center justify-between p-4 rounded-xl bg-surface border border-border">
                    <div className="flex flex-col gap-0.5">
                      ...
                      <p className="text-xs text-muted">
                        Aktivieren, wenn die Live-Links hinter Authentifizierung
                        oder VPN liegen und der Erreichbarkeitscheck immer
                        fehlschlagen würde.
                      </p>
                    </div>
                    <Switch
            ...
            {currentCreateStep === 5 && (
              <div className="space-y-6">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {(formData.techStack || []).map((tech) => (
                          <Chip
                            ...
                            size="sm"
                            variant="soft"
                            className="text-xs font-medium bg-surface-secondary border border-border shadow-sm"
                          >
                            {tech}
                            <button
                              type="button"
                    ...
                    <div className="rounded-3xl border border-border bg-surface p-5">
                      ...
                      <div className="space-y-3">
                        <div className="flex gap-2 flex-wrap">
                          {(
                            ["info", "warning", "danger", "custom"] as const
                          ).map((t) => (
                            <button
                              key={t}
                              type="button"
                        ...
                        <textarea
                          ...
                          onChange={(event) =>
                            setFormData((previous) => ({
                              ...
                              bannerType: event.target.value.trim()
                                ? previous.bannerType || "info"
                                : undefined,
                            }))
                          }
                        />
                      ...
                      <div className="rounded-3xl border border-accent/10 bg-accent/5 p-5">
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            ...
                            <p className="mt-1 text-xs text-muted">
                              Nur für Admins: Empfehlung für hervorgehobene
                              Apps.
                            </p>
                          </div>
                          <Switch
                ...
                <div className="rounded-3xl border border-border bg-surface p-5">
                  ...
                  <p className="mt-1 text-xs text-muted">
                    Nur Felder mit Inhalt erscheinen später in der
                    Detailansicht.
                  </p>
                  <dl className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
                    {metaFields.map((field) => (
                      <div
                        key={field.key}
                        className="rounded-2xl border border-border bg-surface-secondary/40 p-4 transition-colors hover:border-accent/30"
                      >
                        ...
                        <dd className="mt-3">
                          <input
                            ...
                            onChange={(event) => {
                              ...
                              if (value) {
                                if (index >= 0)
                                  fields[index] = { key: field.key, value };
            ...
            {currentCreateStep === 6 && (
              <div className="space-y-6">
                <div className="rounded-3xl border border-border bg-surface-secondary/45 p-5">
                  <div className="flex flex-wrap items-center gap-2">
                    <p className="text-sm font-semibold text-foreground">
                      Markdown für die Detailansicht
                    </p>
                    {gitLabSnapshot?.readmeContent?.trim() && (
                  ...
                  <p className="mt-1 text-xs text-muted">
                    Beschreiben Sie die App ausführlicher. Dieser Schritt bleibt
                    bewusst eigenständig, damit die Dokumentation nicht zwischen
                    andere Angaben gequetscht wird.
                  </p>
                </div>
                <TextField
                  onChange={(value) =>
                ...
                <div className="rounded-3xl border border-border bg-surface p-5">
                  <div className="mb-4">
                    ...
                    <p className="mt-1 text-xs text-muted">
                      Optional: Verknüpfen Sie die App direkt mit bestehenden
                      Einträgen oder ordnen Sie sie einer Gruppe zu.
                    </p>
                  </div>
          ...
          <aside className="space-y-4">
                ...
                <div className="relative mt-4 flex items-start gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-lg font-bold text-foreground">
                      {formData.name || "Neue App"}
                    </p>
                    <p className="mt-1 text-sm text-muted">
                      {formData.description ||
                        "Kurzbeschreibung folgt in einem der nächsten Schritte."}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4 px-5 py-5">
                <div>
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    ...
                    >
                      {getAppStatusLabel(formData.status || DRAFT_STATUS) ||
                        "Entwurf"}
                    </Chip>
                    {formData.isReuse && (
                      <Chip
                    ...
                    {(formData.categories || []).length > 0 ? (
                      (formData.categories || []).map((category) => (
                        <Chip
                          ...
                          size="sm"
                          variant="soft"
                          className="text-[11px] font-medium bg-surface-secondary border border-border"
                        >
                          {category}
                        </Chip>
                      ))
                ...
                <div>
                  ...
                  <p className="mt-2 rounded-xl border border-border bg-surface-secondary px-3 py-2 font-mono text-xs text-foreground">
                    {formData.id || "wird automatisch erstellt"}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-[2rem] border border-border bg-surface px-5 py-5 shadow-sm">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="rounded-full border border-border bg-surface-secondary px-2.5 py-1 text-[11px] font-semibold text-muted">
                  {createChecklist.filter((item) => item.done).length}/
              ...
              <div className="mt-4 space-y-3">
                {createChecklist.map((item) => (
                  <div
                    key={item.label}
                    className={`flex items-start gap-3 rounded-2xl border px-3 py-3 text-sm ${item.done ? "border-success/25 bg-success/10 text-foreground" : "border-border bg-surface-secondary/45 text-muted"}`}
                  >
                    <span
                      className={`mt-0.5 inline-flex h-5 w-5 items-center justify-center rounded-full ${item.done ? "bg-success/15 text-success" : "bg-surface text-muted border border-border"}`}
                    >
                      {item.done ? (
            ...
            <div className="rounded-[2rem] border border-border bg-surface px-5 py-5 shadow-sm">
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
                {requiredItems.map((item) => (
                  <div
                    key={item.label}
                    className={`rounded-2xl border px-4 py-3 text-sm ${item.done ? "border-success/25 bg-success/10 text-foreground" : "border-border bg-surface-secondary/45 text-muted"}`}
                  >
                    ...
                    <p className="mt-1 text-xs">
                      {item.done
                        ? "Erledigt"
                        : "Für den aktuellen Stand noch offen."}
                    </p>
                  </div>
                ))}
              </div>
  ...
  return (
    <div className="max-w-5xl mx-auto pb-20">
      ...
      {formData.bannerText ? (
        <div
          className={`mb-4 px-4 py-3 rounded-xl border flex items-start gap-3 ${
            formData.bannerType === "warning"
              ? "bg-warning/10 border-warning/30"
              : formData.bannerType === "danger"
          ...
          style={
            formData.bannerType === "custom" && formData.bannerColor
              ? {
        ...
        >
          <div className="flex-1 space-y-2">
            ...
            {formData.bannerType === "custom" && (
              <div className="flex items-center gap-2">
                <input
                  ...
                  value={formData.bannerColor || "#6366f1"}
                  onChange={(e) =>
                    setFormData((p) => ({ ...p, bannerColor: e.target.value }))
                  }
                  className="h-8 w-8 shrink-0 rounded cursor-pointer border border-border bg-transparent p-0.5"
                />
                <input
            ...
            <input
              ...
              value={formData.bannerTitle || ""}
              onChange={(e) =>
                setFormData((p) => ({ ...p, bannerTitle: e.target.value }))
              }
            />
            {/* Banner text */}
            <input
              className={`w-full bg-white/40 dark:bg-white/5 border rounded-lg px-3 py-1.5 text-sm text-foreground placeholder:text-muted/50 outline-none transition-colors ${
                formData.bannerType === "warning"
                  ? "border-warning/40 focus:border-warning"
                  : formData.bannerType === "danger"
              ...
              value={formData.bannerText}
              onChange={(e) =>
                setFormData((p) => ({ ...p, bannerText: e.target.value }))
              }
            />
          </div>
          <button
        ...
        <button
          type="button"
          onClick={() =>
            setFormData((p) => ({ ...p, bannerText: " ", bannerType: "info" }))
          }
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-3 mb-3">
              ...
              {(formData.categories || []).map((cat) => (
                <Chip
                  ...
                  variant="soft"
                  color="accent"
                  className="text-[11px] uppercase font-bold tracking-wider group/chip"
                >
                  {cat}
                  <button
                    type="button"
              ...
              {formData.isReuse && (
                <Chip
                  ...
                  variant="soft"
                  color="warning"
                  className="text-[11px] uppercase font-bold flex items-center gap-1"
                >
            ...
            {isNew && (
              <p className="text-[11px] text-muted/60 mb-3 -mt-1">
                Die ID ist die eindeutige Adresse der App und wird automatisch
                aus dem Namen generiert.
              </p>
            )}

            {/* Description (matches detail page position) */}
            <label className="block text-[10px] font-bold text-muted/60 uppercase tracking-wider mb-1.5 select-none">
              Kurzbeschreibung
            </label>
            <textarea
            ...
            <div className="mb-6">
              <label className="block text-[10px] font-bold text-muted/60 uppercase tracking-wider mb-2 select-none">
                Schlagwörter
              </label>
              <div className="flex flex-wrap gap-2 items-center">
                {(formData.tags || []).map((tag) => (
                  <Chip
                    ...
                    size="sm"
                    variant="soft"
                    className="text-xs font-medium bg-surface/50 border-border/60 group/tag"
                  >
                    {tag}
                    <button
                      type="button"
          ...
          <div className="relative z-10 mt-5 p-4 rounded-2xl bg-surface border border-border shadow-lg">
            ...
            <Input
              ...
              onChange={(e) => {
                ...
                if (isImageAssetSource(e.target.value))
                  setFormData((p) => ({ ...p, icon: e.target.value }));
              }}
            />
            {iconUploadError && (
              <p className="text-xs text-danger mt-1">{iconUploadError}</p>
            )}
          </div>
        )}
          ...
          <div className="relative z-10 mt-5 p-4 rounded-2xl bg-surface border border-border shadow-lg">
            ...
            <div className="flex gap-2 items-center">
              <Input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" && categoryInput.trim()) {
                    ...
                    if (!c.includes(categoryInput.trim()))
                      setFormData({
                        ...formData,
                        categories: [...c, categoryInput.trim()],
                      });
                    setCategoryInput("");
                  }
                }}
              ...
              <Button
                ...
                onPress={() => {
                  const c = formData.categories || [];
                  if (categoryInput.trim() && !c.includes(categoryInput.trim()))
                    setFormData({
                      ...formData,
                      categories: [...c, categoryInput.trim()],
                    });
        ...
        <>
            ...
            <div className="flex gap-2 flex-wrap items-center flex-1">
              {(formData.techStack || []).map((tech: string) => (
                <Chip
                  ...
                  size="sm"
                  variant="soft"
                  className="text-xs font-medium bg-surface border border-border shadow-sm group/tech"
                >
                  {tech}
                  <button
                    type="button"
                  ...
                  onKeyDown={(e) => {
                    if (
                      ...
                      if (!stack.includes(techInput.trim())) {
                        setFormData((p) => ({
                          ...p,
                          techStack: [...(p.techStack || []), techInput.trim()],
                        }));
                      }
                      setTechInput("");
                    }
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-muted mb-3">
                    Diese App kann als bestehende Installation mitgenutzt
                    werden. Eine technische Installationsanleitung kann optional
                    trotzdem zusätzlich hinterlegt werden.
                  </p>
                  <div className="mt-3 p-4 rounded-xl bg-surface border border-border">
                    <h4 className="text-xs font-bold text-muted uppercase tracking-wider mb-2">
                      Voraussetzungen zur Nachnutzung
                    </h4>
                    <textarea
                      ...
                      onChange={(e) =>
                        setFormData((p) => ({
                          ...p,
                          reuseRequirements: e.target.value,
                        }))
                      }
                    />
                  </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted/60">
                    Aktivieren, wenn diese App für andere Behörden zur
                    Mitnutzung bereitgestellt wird.
                  </p>
                </div>
              </div>
              <Switch
          ...
          <Tabs variant="secondary" className="w-full">
            ...
            <Tabs.Panel id="docs">
              <div className="space-y-4">
                <p className="text-xs text-muted">
                  Verfassen Sie eine ausführliche Dokumentation im
                  Markdown-Format.
                </p>
                <TextField
                  onChange={(val) =>
                    setFormData((p) => ({ ...p, markdownContent: val }))
                  }
                >
                  <TextArea
                    value={formData.markdownContent || ""}
                ...
                <div className="flex flex-col gap-4 rounded-2xl border border-border/60 bg-surface-secondary/40 p-4">
                  ...
                  >
                    ...
                    <Input
                      ...
                      className="bg-field-background h-8 text-sm font-mono"
                    />
                  </TextField>
                  <TextField
                    onChange={(val) =>
                      setFormData((p) => ({ ...p, changelog: val }))
                    }
                  >
                    <Label className="text-[10px] font-bold text-muted uppercase tracking-wider mb-1">
                      Änderungsprotokoll
            ...
            <Tabs.Panel id="details">
              <div className="space-y-6">
                <p className="text-xs text-muted">
                  Felder ohne Inhalt werden in der App-Detailansicht nicht
                  angezeigt.
                </p>
                <dl className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  {metaFields.map((field) => (
                    <div
                      key={field.key}
                      className="flex flex-col gap-1.5 p-4 rounded-2xl bg-surface-secondary border border-border hover:border-accent/30 transition-colors"
                    >
                      <dt className="text-xs text-muted uppercase tracking-wider font-semibold flex items-center gap-2">
                        {field.icon && (
                          <span className="text-accent">{field.icon}</span>
                      ...
                      <dd>
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (val) {
                              if (idx >= 0)
                                fields[idx] = { key: field.key, value: val };
                ...
                {isAdmin && (
                  <div className="flex items-center justify-between p-4 rounded-xl bg-accent/5 border border-accent/10">
                    <div className="flex flex-col gap-0.5">
                      ...
                      <p className="text-xs text-muted">
                        Hervorgehobene Apps erscheinen mit einer speziellen
                        Markierung im Store.
                      </p>
                    </div>
                    <Switch
                      isSelected={formData.isFeatured || false}
                      onChange={(val) =>
                        setFormData((p) => ({ ...p, isFeatured: val }))
                      }
                    >
                      <Switch.Content>
                        <Switch.Control>
      ...
      <div className="mt-12 pt-4 border-t border-separator flex items-center justify-between text-[11px] text-muted">
        <span>
          ID: <code className="font-mono">{formData.id || "—"}</code>
        </span>
        <span>
          Aktualisiert:{" "}
          {initialApp?.updatedAt
            ? new Date(initialApp.updatedAt).toLocaleDateString("de-DE", {
                day: "2-digit",
                month: "2-digit",
                year: "numeric",
              })
