...
export function AIProviderSettingsPanel() {
  ...
  const [providerToDelete, setProviderToDelete] =
    useState<AIProviderAdminSettings | null>(null);
      ...
      .catch((err) =>
        setError(
          err instanceof Error
            ? err.message
            : "AI-Provider konnten nicht geladen werden.",
        ),
      )
  ...
  const updateProvider = (
    providerKey: string,
    patch: Partial<AIProviderAdminSettings>,
  ) => {
    ...
    try {
      ...
      if (!response.ok)
        throw await parseApiError(
          response,
          "AI-Provider konnte nicht erstellt werden.",
        );
  ...
  const saveProvider = async (
    provider: AIProviderAdminSettings,
    ...
    try {
      const response = await fetchApi(
        `/settings/ai-providers/${provider.providerKey}`,
        {
          method: "PUT",
          body: JSON.stringify({
            ...provider,
            token: options?.clearToken
              ? ""
              : tokenDrafts[provider.providerKey] || "",
      ...
      if (!response.ok)
        throw await parseApiError(
          response,
          "AI-Provider konnte nicht gespeichert werden.",
        );
      setProviders(await loadProviders());
  ...
  const deleteProvider = async () => {
    ...
    try {
      const response = await fetchApi(
        `/settings/ai-providers/${providerToDelete.providerKey}`,
        { method: "DELETE" },
      );
      if (!response.ok)
        throw await parseApiError(
          response,
          "AI-Provider konnte nicht gelöscht werden.",
        );
      setProviders((current) =>
        current.filter(
          (provider) => provider.providerKey !== providerToDelete.providerKey,
        ),
      );
  ...
  const testProvider = async (providerKey: string) => {
    ...
    try {
      const response = await fetchApi(
        `/settings/ai-providers/${providerKey}/test`,
        { method: "POST" },
      );
      if (!response.ok)
        throw await parseApiError(
          response,
          "AI-Provider konnte nicht getestet werden.",
        );
      const data = (await response.json()) as { ok: boolean; message?: string };
      setStatus(
        data.ok
          ? "AI-Provider erreichbar."
          : data.message || "AI-Provider-Test fehlgeschlagen.",
      );
    } catch (err) {
      setError(
        err instanceof Error
  ...
  const runReindex = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "AI-Wissensindex konnte nicht aufgebaut werden.",
      );
    } finally {
      setReindexing(false);
    }
      ...
      <Surface className="border border-border/50 p-6 shadow-sm">
        <div className="mb-5 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-muted">
              Tokens werden verschlüsselt gespeichert und nach dem Speichern
        ...
        {providers.length === 0 ? (
          <div className="rounded-xl border border-warning/20 bg-warning/5 px-4 py-3 text-sm text-warning">
            Es ist aktuell kein AI-Provider konfiguriert.
          </div>
        ) : (
              ...
              >
                <div className="flex flex-col gap-3 border-b border-border pb-4 md:flex-row md:items-start md:justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      size="sm"
                      variant="secondary"
                      isDisabled={testingProvider === provider.providerKey}
                      onPress={() => void testProvider(provider.providerKey)}
                    >
                      {testingProvider === provider.providerKey ? (
                        <Loader2 className="h-4 w-4 animate-spin" />
                      ) : (
                ...
                <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
                  <TextField
                    ...
                    onChange={(value) =>
                      updateProvider(provider.providerKey, { label: value })
                    }
                  >
                    <Label className="mb-1.5 ml-1 text-[10px] font-bold uppercase tracking-widest text-muted">
                      Bezeichnung
                    </Label>
                    <Input
                      className="bg-field-background"
                      placeholder="Interner Chat Provider"
                    />
                  </TextField>
                  <TextField
                    value={provider.baseUrl}
                    onChange={(value) =>
                      updateProvider(provider.providerKey, { baseUrl: value })
                    }
                  >
                    <Label className="mb-1.5 ml-1 text-[10px] font-bold uppercase tracking-widest text-muted">
                      Base URL
                  ...
                  <TextField
                    ...
                    onChange={(value) =>
                      updateProvider(provider.providerKey, { apiPath: value })
                    }
                  >
                    <Label className="mb-1.5 ml-1 text-[10px] font-bold uppercase tracking-widest text-muted">
                      API-Pfad
                  ...
                  <TextField
                    ...
                    onChange={(value) =>
                      updateProvider(provider.providerKey, { chatModel: value })
                    }
                  >
                    <Label className="mb-1.5 ml-1 text-[10px] font-bold uppercase tracking-widest text-muted">
                      Chat-Modell
                  ...
                  <TextField
                    value={tokenDrafts[provider.providerKey] || ""}
                    onChange={(value) =>
                      setTokenDrafts((current) => ({
                        ...current,
                  ...
                  <TextField
                    value={String(provider.temperature)}
                    onChange={(value) =>
                      updateProvider(provider.providerKey, {
                        temperature: Number.parseFloat(value || "0.2") || 0.2,
                      })
                    }
                  >
                    <Label className="mb-1.5 ml-1 text-[10px] font-bold uppercase tracking-widest text-muted">
                    ...
                    <Input
                      ...
                      placeholder="0.2"
                    />
                    <p className="mt-1 ml-1 text-[11px] text-muted">
                      Bestimmt die Kreativität der Antworten. 0 = sehr
                      deterministisch und faktentreu, 1 = ausgewogen, &gt;1 =
                      experimentell und variabler. Empfohlen: 0.1–0.3 für
                      Wissensfragen.
                    </p>
                  </TextField>
                ...
                <div className="mt-5 flex flex-wrap justify-end gap-2 border-t border-border pt-4">
                  <Button
                    ...
                    onPress={() =>
                      void saveProvider(provider, { clearToken: true })
                    }
                  >
                    Token löschen
                  </Button>
            ...
            <Modal.Body>
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                    ...
                    <ListBox>
                      {AI_PROVIDER_TYPES.map((providerType) => (
                        <ListBox.Item
                          key={providerType}
                          id={providerType}
                          textValue={aiProviderTypeLabel(providerType)}
                        >
                ...
                <TextField
                  ...
                  onChange={(value) =>
                    setNewProvider((current) => ({ ...current, label: value }))
                  }
                >
                  <Label className="mb-1.5 ml-1 text-[10px] font-bold uppercase tracking-widest text-muted">
                    Bezeichnung
                ...
                <TextField
                  ...
                  onChange={(value) =>
                    setNewProvider((current) => ({ ...current, token: value }))
                  }
                >
                  <Label className="mb-1.5 ml-1 text-[10px] font-bold uppercase tracking-widest text-muted">
                    API-Key
                  ...
                  <Input
                    className="bg-field-background font-mono text-sm"
                    placeholder="Optional für lokale Provider"
                    type="password"
                  />
                </TextField>
                <TextField
            ...
            <Modal.Footer>
              ...
              <Button
                ...
                isDisabled={
                  ...
                  !newProvider.chatModel.trim()
                }
                onPress={() => void createProvider()}
              >
                {saving ? (
                  <Loader2 className="h-4 w-4 animate-spin" />
                ) : (
