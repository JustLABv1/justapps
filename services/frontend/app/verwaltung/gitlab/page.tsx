...
export default function GitLabSyncPage() {
  ...
  const handleSyncNow = async (appId: string) => {
    ...
    try {
      const res = await fetchApi(`/apps/${appId}/gitlab/sync`, { method: 'POST' });
      if (res.ok) {
    ...
    <div className="space-y-8">
      ...
      <Table variant="secondary">
        <Table.ScrollContainer>
          <Table.Content aria-label="GitLab Sync Status" className="min-w-[860px]">
            <Table.Header>
              ...
              renderEmptyState={() => (
                <EmptyState className="flex flex-col items-center justify-center py-12 gap-2">
                  ...
                  <span className="text-muted">
                    {linkedApps.length === 0
                      ? 'Keine Apps mit GitLab-Verknüpfung gefunden'
