...
const sectionOptions: SectionOption[] = [
  {
    id: "apps",
    label: "Apps",
    description: "App-Stammdaten, Inhalte und Metadaten.",
  },
  {
    id: "appGroups",
    label: "Gruppen",
    description: "Gruppen-Metadaten und Mitgliedschaften.",
  },
  {
    id: "appRelations",
    label: "Relationen",
    description: "Verwandte Apps und bidirektionale Verknüpfungen.",
  },
  {
    id: "users",
    label: "Benutzer",
    description: "Konten, Rollen und Sperrstatus.",
    sensitive: true,
  },
  {
    id: "settings",
    label: "Einstellungen",
    description: "Erscheinungsbild, Detailfelder und Store-Konfiguration.",
  },
  {
    id: "repositoryProviders",
    label: "Repository-Provider",
    description: "Persistierte Provider-Einstellungen.",
  },
  {
    id: "repositoryAppLinks",
    label: "Repository-Linkstatus",
    description: "App-Verknüpfungen, Snapshots und Sync-Status.",
  },
  {
    id: "aiProviders",
    label: "AI-Provider",
    description: "AI-Provider-Einstellungen und verschlüsselte API-Keys.",
  },
  {
    id: "aiConversations",
    label: "AI-Chats",
    description: "Chat-Verläufe und zitierte Quellen.",
    sensitive: true,
  },
  {
    id: "tokens",
    label: "Tokens",
    description: "API- und Zugriffstoken.",
    sensitive: true,
  },
  ...
  },
  {
    id: "assets",
    label: "Uploads",
    description: "Hochgeladene App-Icons, Logos, Favicons und Gruppen-Icons.",
  },
];
...
const sectionPresets = [
  {
    ...
    description:
      "Exportiert alle aktuell angebundenen Datenbereiche als verschlüsselte Backup-Datei. Passwort-Hashes und Token-Secrets werden entfernt.",
    mode: "safe" as const,
  },
  {
    id: "full-fidelity",
    ...
    description:
      "Enthält zusätzlich Passwort-Hashes und Token-Secrets und wird ebenfalls als verschlüsselte Backup-Datei exportiert.",
    mode: "full" as const,
  },
  {
    id: "apps-only",
    ...
    description:
      "Kompatibler, verschlüsselter Abschnittsexport für Apps inklusive referenzierter Uploads, ohne Benutzer- oder Systemeinstellungen.",
    mode: "safe" as const,
...
function normalizeSectionSelection(sections: string[]) {
  ...
  const requiresAssets = assetDependentSectionIds.some((sectionId) =>
    uniqueSections.includes(sectionId),
  );
  if (!requiresAssets) {
...
export default function BackupsPage() {
  ...
  const [pendingExport, setPendingExport] =
    useState<PendingExportAction | null>(null);
  ...
  const submitExport = async () => {
    ...
    if (!exportReady) {
      toast.warning(
        "Bitte geben Sie eine Backup-Passphrase mit mindestens 12 Zeichen ein und bestätigen Sie diese.",
      );
      return;
    }
    ...
    try {
      ...
      const suffix =
        pendingExport.sections.length === 1
          ? pendingExport.sections[0]
          : pendingExport.sections.length < sectionOptions.length
            ? "selected"
            : "instance";
      anchor.href = url;
      anchor.download = parseDownloadFilename(
        response.headers.get("Content-Disposition"),
        `justapps-backup-${suffix}-${pendingExport.mode}-${new Date().toISOString().split("T")[0]}.jabackup`,
      );
  ...
  const handleImportFile = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedSections.length === 0) {
      toast.warning(
        "Mindestens ein Abschnitt muss für den Import ausgewählt sein.",
      );
      return;
    }
    if (replaceBlocked) {
      toast.warning(
        "Replace-Import ist aktuell nur mit vollständiger Abschnittsauswahl möglich.",
      );
      return;
    }
  ...
  const submitImport = async () => {
    ...
    try {
      ...
      const upload = (await createResponse
        .json()
        .catch(() => null)) as BackupUpload | null;
      if (!createResponse.ok || !upload?.uploadId || !upload.chunkSize) {
        throw new Error(
          getRequestError(
            upload,
            "Backup-Upload konnte nicht gestartet werden.",
          ),
        );
      }
      ...
      while (offset < pendingImportFile.size) {
        const chunk = pendingImportFile.slice(
          offset,
          offset + upload.chunkSize,
        );
        const chunkResponse = await fetchApi(
          `/admin/backups/import/uploads/${encodeURIComponent(uploadId)}?offset=${offset}`,
          {
            method: "PUT",
        ...
        if (!chunkResponse.ok) {
          throw new Error(
            chunkResult?.detail ||
              chunkResult?.error ||
              "Ein Backup-Teil konnte nicht hochgeladen werden.",
          );
        }
        offset += chunk.size;
        setImportProgress(
          Math.min(99, Math.round((offset / pendingImportFile.size) * 100)),
        );
      }

      const response = await fetchApi(
        `/admin/backups/import/uploads/${encodeURIComponent(uploadId)}/complete`,
        {
          method: "POST",
          body: JSON.stringify({
            restoreMode,
      ...
      const job = (await response
        .json()
        .catch(() => null)) as BackupImportJob | null;
      if (!response.ok || !job?.jobId) {
        throw new Error(
          getRequestError(job, "Backup konnte nicht gestartet werden."),
        );
      }
      uploadId = "";
      ...
      while (!result) {
        ...
        const statusResponse = await fetchApi(
          `/admin/backups/import/jobs/${encodeURIComponent(job.jobId)}`,
          {
            cache: "no-store",
          },
        );
        const status = (await statusResponse
          .json()
          .catch(() => null)) as BackupImportJobStatus | null;
        if (!statusResponse.ok || !status) {
          throw new Error(
            getRequestError(
              status,
              "Importstatus konnte nicht geladen werden.",
            ),
          );
        }
      ...
      if (uploadId) {
        await fetchApi(
          `/admin/backups/import/uploads/${encodeURIComponent(uploadId)}`,
          {
            method: "DELETE",
          },
        ).catch(() => undefined);
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-2">
        ...
        <p className="max-w-3xl text-sm text-muted">
          Der Backup-Bereich exportiert jetzt verschlüsselte Sicherungsdateien
          mit klar markierten Datenabschnitten. Exporte und Importe laufen über
          dieselbe Oberfläche, damit vollständige Sicherungen und selektive
          Wiederherstellungen nicht mehr über einzelne Verwaltungsseiten
          verteilt sind.
        </p>
      </div>
      ...
      <Surface className="border border-warning/25 bg-warning/5 p-5 shadow-sm">
        <div className="flex items-start gap-3">
          ...
          <div className="space-y-2 text-sm text-muted">
            ...
            <p>
              Der sichere Modus entfernt sensible Geheimnisse.
              Legacy-JSON-Backups können weiter importiert werden, werden dabei
              aber explizit gewarnt. Replace-Restores bleiben absichtlich auf
              Vollsicherungen beschränkt.
            </p>
          </div>
        </div>
      </Surface>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        {sectionPresets.map((preset) => {
          ...
          return (
            <Surface
              key={preset.id}
              className="flex h-full flex-col gap-4 border border-border/60 p-6 shadow-sm"
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs uppercase tracking-[0.2em] text-muted">
                    {preset.mode === "safe" ? "Empfohlen" : "Sensitiv"}
                  </p>
                </div>
              </div>
              ...
              <Button
                ...
                onPress={() =>
                  openExportModal(preset.mode, preset.id, preset.sections)
                }
        ...
        <div className="flex flex-col gap-6">
          <div className="flex flex-col gap-2">
            <h2 className="text-lg font-semibold text-foreground">
              Selektiver Export und Import
            </h2>
            <p className="max-w-3xl text-sm text-muted">
              Wählen Sie gezielt Abschnitte aus, exportieren Sie nur den
              benötigten Scope oder spielen Sie eine bestehende Sicherung im
              Merge- oder Replace-Modus zurück.
            </p>
          </div>
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1.4fr)_minmax(18rem,0.8fr)]">
            <div className="space-y-4">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <Button size="sm" variant="secondary" onPress={clearSections}>
                  Auswahl leeren
                </Button>
                <p className="text-xs uppercase tracking-[0.2em] text-muted">
                  {selectedSections.length} von {sectionOptions.length}{" "}
                  Abschnitten ausgewählt
                </p>
              </div>
                    ...
                    <label
                      ...
                      className={`flex cursor-pointer items-start gap-3 rounded-2xl border p-4 transition-colors ${
                        isSelected
                          ...
                          : "border-border/60 bg-surface-secondary/40 hover:bg-surface-secondary"
                      }`}
                    >
                      <input
                        type="checkbox"
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          {section.sensitive ? (
                            <span className="rounded-full bg-warning/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.2em] text-warning">
                              Sensitiv
                            </span>
                          ) : null}
                        </div>
            ...
            <div className="space-y-4 rounded-3xl border border-border/60 bg-surface-secondary/30 p-5">
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted">
                  {isFullMode
                    ? "Vollständiger Modus mit sensiblen Secrets."
                    : "Sicherer Modus ohne Passwort-Hashes und Token-Secrets."}
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                {replaceBlocked ? (
                  <p className="text-sm font-medium text-warning">
                    Replace ist nur mit vollständiger Abschnittsauswahl
                    verfügbar.
                  </p>
                ) : null}
              </div>
              ...
              <div className="flex flex-col gap-3">
                <Button
                  ...
                  isDisabled={
                    Boolean(downloadingId) || selectedSections.length === 0
                  }
                >
                  {downloadingId === "advanced-export" ? (
                    <Loader2 className="h-4 w-4 animate-spin" />
                  ) : (
                    <Download className="h-4 w-4" />
                  )}
                ...
                <input
                  ref={fileInputRef}
                  type="file"
                  accept="application/json,.json,.jabackup"
                  className="hidden"
                ...
                <Button
                  ...
                  isDisabled={
                    importing || selectedSections.length === 0 || replaceBlocked
                  }
                >
                  {importing ? (
                    <Loader2 className="h-4 w-4 animate-spin" />
                ...
                <p className="text-sm text-muted">
                  {selectedFileName
                    ? `Letzte Datei: ${selectedFileName}`
                    : "Die Passphrase wird nach dem Klick und der Dateiauswahl in einem Modal abgefragt."}
                </p>
              </div>
            </div>
          </div>
        ...
        <Surface className="border border-border/60 p-6 shadow-sm">
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-muted">
                {importResult.message} · Modus: {importResult.restoreMode}
              </p>
            </div>
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {Object.entries(importResult.stats).map(([section, stats]) => (
                <div
                  key={section}
                  className="rounded-2xl border border-border/60 bg-surface-secondary/40 p-4"
                >
                  <p className="text-xs font-bold uppercase tracking-[0.2em] text-muted">
                    {section}
                  </p>
            ...
            <Modal.Dialog className="sm:max-w-md">
              ...
              <Modal.Body>
                <div className="space-y-5">
                  <p className="text-sm text-muted">
                    Die ausgewählte Sicherung wird erst nach Eingabe einer
                    Passphrase exportiert. Ohne diese Passphrase kann das Backup
                    später nicht wiederhergestellt werden.
                  </p>
                  {pendingExport ? (
                    <p className="text-xs font-bold uppercase tracking-[0.2em] text-muted">
                      {pendingExport.sections.length} Abschnitt
                      {pendingExport.sections.length !== 1 ? "e" : ""} ·{" "}
                      {pendingExport.mode === "safe"
                        ? "Sicherer Modus"
                        : "Vollmodus"}
                    </p>
                  ) : null}
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        className="w-full"
                        onChange={(event) =>
                          setExportPassphraseConfirm(event.target.value)
                        }
                  ...
                  <p
                    className={`text-sm ${exportReady ? "text-success" : "text-muted"}`}
                  >
                    {!exportPassphrase
              ...
              <Modal.Footer>
                <div className="flex w-full justify-end gap-3">
                  ...
                  <Button
                    ...
                    isDisabled={!exportReady || Boolean(downloadingId)}
                  >
                    {downloadingId ? (
                      <Loader2 className="h-4 w-4 animate-spin" />
                    ) : (
            ...
            <Modal.Dialog className="sm:max-w-md">
              ...
              <Modal.Body>
                <div className="space-y-5">
                  <p className="text-sm text-muted">
                    {pendingImportFile
                      ? `Datei: ${pendingImportFile.name}`
                      : "Nach der Dateiauswahl kann der Import hier gestartet werden."}
                  </p>
                  <div className="space-y-2">
                    <p className="text-xs font-bold uppercase tracking-[0.2em] text-muted">
                      Passphrase
                  ...
                  <p className="text-sm text-muted">
                    Für `.jabackup`-Dateien ist die Passphrase erforderlich.
                    Legacy-JSON-Backups können ohne Passphrase importiert werden
                    und erzeugen einen Warnhinweis.
                  </p>
                  {importing ? (
                    <div className="space-y-2">
                      <div className="flex items-center justify-between text-xs text-muted">
                        <span>
                          {importPhase === "uploading"
                            ? "Backup wird in kleinen Teilen übertragen"
              ...
              <Modal.Footer>
                <div className="flex w-full justify-end gap-3">
                  ...
                  <Button
                    ...
                    isDisabled={importing || !pendingImportFile}
                  >
                    {importing ? (
                      <Loader2 className="h-4 w-4 animate-spin" />
                    ) : (
