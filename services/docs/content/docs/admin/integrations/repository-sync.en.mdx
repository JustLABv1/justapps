...
description: Sync app metadata and deployment content from GitLab and GitHub repositories.
---
...
2. An admin creates one or more **repository providers** (instance URL + access token + provider type) under [**Verwaltung → Integrationen → Repository-Provider**](/verwaltung/integrationen/repository-providers).
...
Provider definitions live in the database and are managed in the [repository provider settings](/verwaltung/integrationen/repository-providers). Each provider has:
...
| `autoSyncEnabled`        | Whether linked apps are synchronized on the scheduler                                               | `true`                          |
...
| `defaultReadmePath`      | Default README path used when an app link does not override it                                      | `README.md`                     |
| `defaultHelmValuesPath`  | Default Helm values path used during import                                                         | `chart/values.yaml`             |
| `defaultComposeFilePath` | Default Compose file used during import                                                             | `docker-compose.yml`            |
